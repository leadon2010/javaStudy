...
public class MemberDAO {
	...
	private static final String USER = "hr"; // DB ID
	private static final String PASS = "hr"; // DB 패스워드
	MemberList mList;
	...
	public boolean insertMember(MemberDTO dto) {
		...
		try {
			...
			con = getConn();
			String sql = "insert into member(" + "id,pwd,name,tel,addr,birth," + "job,gender,email,intro) "
					+ "values(?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean updateMember(MemberDTO vMem) {
		...
		try {
			...
			con = getConn();
			String sql = "update member set name=?, tel=?, addr=?, birth=?, job=?, gender=?" + ", email=?,intro=? "
					+ "where id=? and pwd=?";
